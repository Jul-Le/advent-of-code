 ...
 *
 *
 *  Find the board that is going to find last.
 *
...
public:
    ...
    bool win()
    {
        // If board has won once, m_win will be set to true so the
        // following checks don't have to be executed every time
        if (m_win)
            return true;
...
int main()
{
    ...
    for (int i = 2; i < input.size(); i++)
    {
        ...
        boards.push_back(new BingoBoard(numbers));
        i += 5;
    }
    
    BingoBoard *losingBoard;
    int lastNum;
    const int boardCount = boards.size();
    int boardsLeft = boardCount;
    ...
    for (auto num : numbers)
    {
        ...
        {
            boardsLeft = boardCount;
            continue;
        }
    }
        
    std::cout << "Sum of unmarked numbers is " << losingBoard->unmarkedSum() << std::endl
              << "The last number was " << lastNum << std::endl
              << "The result is " << losingBoard->unmarkedSum() * lastNum << std::endl;
