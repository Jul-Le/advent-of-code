 ...
 *  override wire "b" to that signal, and reset the other
...
int main()
{
    ...
    uint16_t signal = wires["a"];
    
    // Replace "b" value in input and emulate again
    for (std::string &instruction : input)
    {
        if (instruction.find(" -> b") != std::string::npos)
        {
            instruction = std::to_string(signal) + " -> b";
            break;
        }
    }
