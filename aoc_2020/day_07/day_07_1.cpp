 ...
 *  
 *
 *  Input specifies contents of different bags.
 *
 *  What bags contain at least one gold bag (directly
 *  or indirectly)?
 */
...
int main()
{
    ...
    int countOfBagsThatContainShinyGoldBag = 0;
    ...
    for (auto bag : bags)
    {
        if (bag.containsBag(/* shiny gold */))
            countOfBagsThatContainShinyGoldBag++;
    }
    
    std::cout << "There are "<< countOfBagsThatContainShinyGoldBag
              << " bags that contain at least one shiny gold bag." << std::endl;
